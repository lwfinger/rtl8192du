...
u8 rtw_setopmode_cmd(struct rtw_adapter *padapter, enum NDIS_802_11_NETWORK_INFRASTRUCTURE networktype)
{
	...
	psetop = kzalloc(sizeof(struct setopmode_parm), GFP_ATOMIC);
	if (psetop == NULL) {
		kfree(ph2c);
		res = false;
		goto exit;
...
u8 rtw_setrttbl_cmd(struct rtw_adapter *padapter, struct setratable_parm *prate_table)
{
	...
	psetrttblparm = kzalloc(sizeof(struct setratable_parm), GFP_ATOMIC);
...
u8 rtw_getrttbl_cmd(struct rtw_adapter *padapter, struct getratable_rsp *pval)
{
	...
	pgetrttblparm = kzalloc(sizeof(struct getratable_parm), GFP_ATOMIC);
...
u8 rtw_setassocsta_cmd(struct rtw_adapter *padapter, u8 *mac_addr)
{
	...
	psetassocsta_rsp = kzalloc(2 * sizeof(struct set_assocsta_rsp), GFP_ATOMIC);
...
u8 rtw_set_chplan_cmd(struct rtw_adapter *padapter, u8 chplan, u8 enqueue)
{
	...
	/* prepare cmd parameter */
	setchannelplan_param = kzalloc(sizeof(struct setchannelplan_param), GFP_ATOMIC);
...
static void c2h_wk_callback(_workitem *work)
{
	...
	while (!rtw_cbuf_empty(evtpriv->c2h_queue)) {
		...
		} else if ((c2h_evt = kzalloc(16, GFP_ATOMIC)) != NULL) {
