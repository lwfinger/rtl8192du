                            ...
                            union iwreq_data *wrqu, char *extra)
{
	...
	p = &wrqu->data;
	len = p->length;
	ptmp = kzalloc(len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (!ptmp)
		return -ENOMEM;
...
struct	iw_point
{
  ...
  __u16		flags;		/* Optional params */
	...
	len = p->length;
	pparmbuf = kzalloc(len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (!pparmbuf){
		ret = -ENOMEM;
		goto _rtw_drvext_hdl_exit;
	}
						...
						union iwreq_data *wrqu, char *extra)
{
	...
	char *buffer = kzalloc(length, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
...
static int wpa_supplicant_ioctl(struct net_device *dev, struct iw_point *p)
{
	...
	param = kzalloc(p->length, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (!param) {
		ret = -ENOMEM;
		goto out;
	}
...
static int rtw_set_wps_beacon(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ie_len > 0) {
		pmlmepriv->wps_beacon_ie = kzalloc(ie_len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
...
static int rtw_set_wps_probe_resp(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if(ie_len>0)
	{
		pmlmepriv->wps_probe_resp_ie = kzalloc(ie_len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
...
static int rtw_set_wps_assoc_resp(struct net_device *dev, struct ieee_param *param, int len)
{
	...
	if (ie_len > 0) {
		pmlmepriv->wps_assoc_resp_ie = kzalloc(ie_len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
				...
				char *extra)
{
	...
	if(dwrq->flags == 0x8766 && len > 8)
	{
			...
			(_rtw_memcmp(&probereq_wpsie[2], wps_oui, 4) ==_TRUE))
		{
			...
			pmlmepriv->wps_probe_req_ie = kzalloc(cp_sz, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
			...
			union iwreq_data *wdata, char *extra)
{
	...
	err = 0;
	data = kzalloc(EFUSE_BT_MAX_MAP_LEN, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (data == NULL) {
		err = -ENOMEM;
		goto exit;
	}
	rawdata = kzalloc(EFUSE_BT_MAX_MAP_LEN, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (rawdata == NULL) {
		err = -ENOMEM;
		goto exit;
	}
			...
			union iwreq_data *wdata, char *extra)
{
	...
	pEfuseHal = &pHalData->EfuseHal;
	err = 0;
	setdata = kzalloc(1024, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (setdata == NULL)
	{
		err = -ENOMEM;
		goto exit;
	}
	ShadowMapBT = kzalloc(EFUSE_BT_MAX_MAP_LEN, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (ShadowMapBT == NULL)
	{
		err = -ENOMEM;
		goto exit;
	}
	ShadowMapWiFi = kzalloc(EFUSE_MAP_SIZE, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (ShadowMapWiFi == NULL)
	{
		err = -ENOMEM;
		goto exit;
	}
	setrawdata = kzalloc(EFUSE_MAX_SIZE, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (setrawdata == NULL)
	{
		err = -ENOMEM;
		goto exit;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	pbuf = kzalloc(sizeof(l2_msg_t), in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if(pbuf) {
...
static s32 initLoopback(PADAPTER padapter)
{
	...
	if (padapter->ploopback == NULL) {
		ploopback = kzalloc(sizeof(LOOPBACKDATA), in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	...
	union iwreq_data *wrqu, char *extra)
{
	...
	pbuf = kzalloc(len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (pbuf == NULL) {
		printk("%s: no memory!\n", __func__);
		return -ENOMEM;
	}
...
static int rtw_ioctl_wext_private(struct net_device *dev, union iwreq_data *wrq_data)
{
	...
	input = kzalloc(input_len, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	...
	buffer = kzalloc(4096, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
	if (NULL == buffer) {
		err = -ENOMEM;
		goto exit;
	}
	...
	* will have to handle the data */
		...
		(priv_args[k].get_args & IW_PRIV_SIZE_MASK))
	{
		...
		output = kzalloc(4096, in_interrupt() ? GFP_ATOMIC : GFP_KERNEL);
		if (NULL == output) {
			err =  -ENOMEM;
			goto exit;
		}
